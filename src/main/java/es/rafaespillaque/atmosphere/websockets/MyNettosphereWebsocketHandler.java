...
public class MyNettosphereWebsocketHandler implements WebSocketProtocol {
    ...
    public void configure(AtmosphereConfig config) {
        System.out.println(String.format("configure(): {Config: %s}", config));
    }
                ...
                final Future<?> future = b.scheduleFixedBroadcast(
                        new Callable<String>() {
                            ...
                            public String call() throws Exception {
                                ...
                                if(b.getAtmosphereResources().size() == 0) {
                                    System.out.println("No resources attached. Destroying broadcaster.");
                                    futures.remove(path);
                                    b.destroy();
                                    return null;
                                }
    ...
    public List<AtmosphereRequest> onMessage(WebSocket ws, String message) {
        ...
        try {
			...
			String action = jObj.getAsJsonPrimitive("type").getAsString();
			if (action.equals("uuid")) {
				b.broadcast(String.format("{action:uuid,uuid=%s}", ws.resource().uuid()));
				ws.write(String.format("{action:uuid,uuid=%s}", ws.resource().uuid()));
    ...
    public void onClose(WebSocket webSocket) {
        System.out.println(String.format("onClose(): {IP: %s} : {Port: %s}",
                webSocket.resource().getRequest().getRemoteAddr(),
    ...
    public void onError(WebSocket webSocket, WebSocketProcessor.WebSocketException ex) {
        ...
        System.out.println(String.format(ex.getMessage() + " Status {%s} Message {%s}",
                webSocket.resource().getResponse().getStatus(),
                ex.response().getStatusMessage()));
    }
}
